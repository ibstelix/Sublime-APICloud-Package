...
class BuildApkCommand(sublime_plugin.WindowCommand):
    ...
    def checkBasicInfo(self):
        ...
        if 'darwin' in platform.system().lower():
            ...
            if not os.path.exists(os.path.join(self.__curDir,'tools','linux')) or not os.path.isdir(os.path.join(self.__curDir,'tools','linux')):
                self.__apkLogging.info('checkBasicInfo:cannot find tools')
                return -1        
        else:
    ...
    def decompileApk(self):
        ...
        if 'darwin' in platform.system().lower() :
            ...
            apkToolsPath='"'+os.path.join('/usr/bin/','apktool')+'"'
        elif 'windows' in platform.system().lower(): 
            apkToolsPath='"'+os.path.join(self.__curDir,'tools','mac','apktool.jar')+'"'
        else:
            return
        ...
        unzipApk=apkToolsPath+ " d -f -s " + sourceApk + " -o " + cachePath;
    ...
    def copyWidget(self, srcPath):
        self.__apkLogging.info ('copyWidget: begin copyWidget - srcPath: '+srcPath)
    ...
    def packageApk(self, srcPath):
        self.__apkLogging.info ('packageApk: begin packageApk - srcPath'+srcPath)
        ...
        if 'darwin' in platform.system().lower():
            ...
            apkToolsPath='"'+os.path.join('/usr/bin/','apktool')+'"'
            aaptPath='"'+os.path.join('/usr/bin/','aapt')+'"'
        elif 'windows' in platform.system().lower(): 
            apkToolsPath='"'+os.path.join(self.__curDir,'tools','mac','apktool.jar')+'"'
        ...
        if 'darwin' in platform.system().lower():    
            ...
            unsignedApk='"'+os.path.join(self.__cachePath,self.__appId+'.unsigned')+'"'
            unsignedApkCmd=apkToolsPath+' b '+ ' "'+os.path.join(self.__cachePath,'android')+'" -o '+unsignedApk
        else:
            unsignedApk='"'+os.path.join(self.__cachePath,self.__appId+'.unsigned')+'"'
            unsignedApkCmd=apkToolsPath+' b '+ ' "'+os.path.join(self.__cachePath,'android')+'" -o '+unsignedApk
        ...
        if 'logFile'==self.__cmdLogType or 'darwin' in platform.system().lower() or 'linux' in platform.system().lower():
        ...
        if 'darwin' in platform.system().lower() :
            ...
            toolsPath='"'+os.path.join(self.__curDir,'tools','linux','tools.jar')+'"'
            keyFilePath='"'+os.path.join(self.__curDir,'tools','linux','uzmap.keystore')+'"'    
        elif 'windows' in platform.system().lower(): 
            toolsPath='"'+os.path.join(self.__curDir,'tools','mac','tools.jar')+'"'
            keyFilePath='"'+os.path.join(self.__curDir,'tools','mac','uzmap.keystore')+'"'
        ...
        if 'darwin' in platform.system().lower() :
            ...
            zipAlignPath='"'+os.path.join(self.__curDir,'tools','linux','23.0.2','zipalign')+'"'  
        elif 'windows' in platform.system().lower(): 
    ...
    def buildApk(self,path):
        ...
        sublime.message_dialog(u'_构建成功 !')    
...
class EnBuildApkCommand(sublime_plugin.WindowCommand):
    """docstring for BuildApkCommand"""
    __curDir=''
    __appId=''
    __cachePath=''
    ...
    def runShellCommand(self, cmd):
        ...
        stdout=''
        stderr=''
        self.__apkLogging.info('runShellCommand: executing cmd: ' + cmd)
        ...
        if 'darwin' in platform.system().lower():
            ...
            self.__apkLogging.info('runShellCommand: the platform is not support - cmd: '+cmd)
        return (rtnCode,stdout,stderr)  
    ...
    def buildApk(self,path):
        self.__apkLogging.info('buildApk: begin buildApk - path'+path)
        if -1==self.checkBasicInfo():
            sublime.error_message(u'Package missing file')
            return
        self.getWidgetInfo(path)
        self.decompileApk()
        self.modifyAndroidManifest()
        ...
        rtnCode=self.packageApk(path)
        if -1==rtnCode:
            sublime.error_message(u'Packaging failed')
        elif -2==rtnCode:
            sublime.error_message(u'Apk package of the same name already exists, please remove the old version of the package')
        sublime.message_dialog(u'Build Complete !')    
        if os.path.exists(os.path.join(self.__cachePath,'android')):
            CleanDir(os.path.join(self.__cachePath,'android'))  
        if os.path.exists(os.path.join(self.__cachePath,self.__appId+'.unsigned')):
            os.remove( os.path.join(self.__cachePath,self.__appId+'.unsigned')) 
...
class FrBuildApkCommand(sublime_plugin.WindowCommand):
    """docstring for BuildApkCommand"""
    __curDir=''
    __appId=''
    __cachePath=''
    ...
    def decompileApk(self):
        ...
        if 'darwin' in platform.system().lower() :
            ...
            apkToolsPath='"'+os.path.join(self.__curDir,'tools','linux','apktool.jar')+'"'
        elif 'windows' in platform.system().lower(): 
            apkToolsPath='"'+os.path.join(self.__curDir,'tools','mac','apktool.jar')+'"'
        else:
            return
    ...
    def buildApk(self,path):
        self.__apkLogging.info('buildApk: begin buildApk')
        if -1==self.checkBasicInfo():
            sublime.error_message(u'Fichiers manquants pour la creation du package')
            return
        self.getWidgetInfo(path)
        self.decompileApk()
        self.modifyAndroidManifest()
        ...
        rtnCode=self.packageApk(path)
        if -1==rtnCode:
            sublime.error_message(u'Echec de l''Operation.')
        elif -2==rtnCode:
            sublime.error_message(u'Un Apk du même nom déjà existe, veuillez supprimer l''ancienne version du paquet')

        sublime.message_dialog(u'Apk generE avec succes !')    
        if os.path.exists(os.path.join(self.__cachePath,'android')):
            CleanDir(os.path.join(self.__cachePath,'android'))  
        if os.path.exists(os.path.join(self.__cachePath,self.__appId+'.unsigned')):
            os.remove( os.path.join(self.__cachePath,self.__appId+'.unsigned')) 
